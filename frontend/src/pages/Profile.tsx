...
const Profile = () => {
  const { posts, toggleUpvote, toggleBookmark, currentUser: ctxUser, isAnonymousMode, toggleAnonymousMode, refreshProfile, isFollowing: checkIsFollowing, followUser, unfollowUser } = usePosts();
  ...
  // Use global context for follow status
  ...
  const handleFollow = async () => {
    try {
      ...
      setStats(prev => ({ ...prev, followers: prev.followers + 1 })); // Local optimistic update for stats
  ...
  const handleUnfollow = async () => {
    try {
      ...
      setStats(prev => ({ ...prev, followers: Math.max(0, prev.followers - 1) })); // Local optimistic update for stats
