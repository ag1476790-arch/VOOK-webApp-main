...
const Chats = () => {
  ...
  // 1. Fetch Chats
  // 1. Fetch Chats (Optimized with Promise.all)
  const fetchChats = async () => {
    if (!currentUser?.id) {
      setLoading(false); // Fix: Ensure loading is false if no user
      return;
    }
    ...
    try {
      ...
      const [chatsDataResult, partnersResult, messagesResult] = await Promise.all([
      ...
      if (chatsDataResult.error) throw chatsDataResult.error;
      if (partnersResult.error) console.error("Partner fetch error:", partnersResult.error);
      if (messagesResult.error) throw messagesResult.error;

      const chatsData = chatsDataResult.data || [];
      ...
      partnersData.forEach((p: any) => {
        partnersMap.set(p.chat_id, p.profiles);
      });
      ...
      const formattedChats: ChatListItem[] = chatsData.map((c: any) => {
        ...
        return {
          ...
          lastMessage: lastMsg ? (
            (lastMsg.content.includes('giphy') || lastMsg.content.includes('tenor')) ? 'Sent a GIF' :
      ...
      const uniqueChats: ChatListItem[] = [];
        ...
        if (chat.isGroup) {
          ...
          if (chat.userId) {
            if (!seenUserIds.has(chat.userId)) {
              ...
              // Check if we should swap for a better version (one with messages)
              const existingIndex = uniqueChats.findIndex(c => c.userId === chat.userId);
              if (existingIndex !== -1) {
                const existing = uniqueChats[existingIndex];
                const existingHasMsg = existing.lastMessage !== "Start a conversation";
                const currentHasMsg = chat.lastMessage !== "Start a conversation";
                if (!existingHasMsg && currentHasMsg) {
                  uniqueChats[existingIndex] = chat;
                }
              }
            }
          } else {
  ...
  const handleStartNewChat = async (targetUser: SearchUser) => {
    ...
    try {
      // Check if a PRIVATE chat already exists with this user (via RPC for reliability)
      const { data: existingChatId, error: rpcError } = await supabase
        .rpc('get_private_chat', { other_user_id: targetUser.id });

      if (rpcError) console.error("RPC Error:", rpcError);

      if (existingChatId) {
        // Chat exists, load it
        const existingChatUI = chats.find(c => c.chatId === existingChatId);
        if (existingChatUI) {
          setActiveChat(existingChatUI);
        } else {
          // Construct temp object if not in list yet
          setActiveChat({
            ...
            <Virtuoso
              ...
              itemContent={(index, chat) => (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  className={cn(
                    "group flex items-center gap-3 p-3 rounded-md cursor-pointer transition-all relative mb-1",
