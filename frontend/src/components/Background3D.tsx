...
const Background3D = () => {
    ...
    useEffect(() => {
        ...
        // Reduced particle count significantly for performance (Cap at 60)
        const particleCount = Math.min(Math.floor((width * height) / 15000), 60);
        const connectionDistance = 120; // Reduced distance
        const mouseParams = { x: -1000, y: -1000, radius: 120 }; // Init off-screen
            ...
            for (let i = 0; i < particles.length; i++) {
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (distance < connectionDistance) {
                        ...
                        ctx.strokeStyle = `rgba(29, 155, 240, ${opacity * 0.2})`; // Lower opacity
                        ctx.beginPath();
        ...
        const handleResize = () => {
            ...
            particles.length = 0;
            const newCount = Math.min(Math.floor((width * height) / 15000), 60);
            for (let i = 0; i < newCount; i++) {
                particles.push(new Particle());
            }
            // Recalculate aspect ratio independent stuff if needed
        };
