...
export const PostProvider = ({ children }: { children: ReactNode }) => {
    ...
    // --- Realtime Subscriptions (with retry logic and error handling) ---
    const subscriptionsRef = useRef<RealtimeSubscription[]>([]);
    const authSubscriptionRef = useRef<{ unsubscribe: () => void } | null>(null);

    useEffect(() => {
        // Posts subscription
        const postsSub = new RealtimeSubscription({
            channel: 'public:posts',
            table: 'posts',
            event: '*',
            onInsert: () => queryClient.invalidateQueries({ queryKey: ['posts'] }),
            onUpdate: () => queryClient.invalidateQueries({ queryKey: ['posts'] }),
            onDelete: () => queryClient.invalidateQueries({ queryKey: ['posts'] }),
            onError: (error) => console.error('Posts subscription error:', error),
        });

        // Notifications subscription
        const notifSub = new RealtimeSubscription({
            ...
            onError: (error) => console.error('Notifications subscription error:', error),
        });

        // Messages subscription
        const msgSub = new RealtimeSubscription({
            channel: 'global_unread_messages',
            table: 'messages',
            event: '*',
            onInsert: () => queryClient.invalidateQueries({ queryKey: ['messages', 'unread'] }),
            onUpdate: () => queryClient.invalidateQueries({ queryKey: ['messages', 'unread'] }),
            onError: (error) => console.error('Messages subscription error:', error),
        });
        ...
        Promise.all([
            ...
            console.error('Failed to subscribe to realtime channels:', error);
        });

        subscriptionsRef.current = [postsSub, notifSub, msgSub];
